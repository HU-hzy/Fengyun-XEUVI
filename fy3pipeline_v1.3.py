...
ifflat=1#是否去除平场
if_rmnoise=0#是否去除噪声
indir = '/data1/huzy/xeuvi/04/4.17/'#hdf地址
outdir = '/data1/huzy/xeuvi/xeuvi_out1.3/4.17/'
flatsdir = outdir+'/flats/'
outfitsdir0 = outdir+'/fits0/'#未去除弯曲网格的路径
...
for orbit_i in range(orbit_num):
    IM=np.zeros((1,1072,1032))  # 用于存储直接读取的原始图像
    timelist=[]
    ...
    #保存平场
    np.save(flatsdir+'{}_orbit_{}_flat'.format(timelist[0].strftime('%Y%m%d'),orbit_i),(flatim[::-1,:]).T)
    fits.writeto(flatsdir+'{}_orbit_{}_flat.fits'.format(timelist[0].strftime('%Y%m%d'),orbit_i),flatim)
    all_timelist.extend(timelist)
    fulldisk_num=len(timelist)
    print('该轨有'+str(fulldisk_num)+'张完整太阳像,共{}张。'.format(num0))
    ...
    #print('将第'+str(reference_i)+'张XEUVI图像与AIA图像对齐')
    #aia_map=give_aiamap(timelist[reference_i],aia_dir=aia_dir,method=down_method) # 下载AIA图像，并做预处理
    data = aia_map.data/aia_map.exposure_time  # 对AIA数据归一化
    header = aia_map.fits_header
    ...
    print('一致性亮度定标')
    med_a=np.median(aia_map.data[362:662,362:662])#/aia_map.exposure_time  #选取中间300*300的区域
    med_fy=np.median(im0[reference_i,362:662,362:662])
    coe=med_fy/med_a                                   #计算与aia比值
    for k in range(0,np.shape(im0)[0]):
        im0[k,:,:]=im0[k,:,:]*coe                        #定标
    im = im0
    if if_rmnoise:
        im = rm_noise(im)
    ...
    for i in tqdm(range(fulldisk_num)):
        fy3_writefits((im[i][::-1,:]).T,outfitsdir0,'',Scale=scale0,obstime=timelist[i],R_SUN=re[i][2],rmghost=0,coe=coe)

    im = rm1(im)#去除弯曲网格
    print('开始保存fits')
    for i in tqdm(range(fulldisk_num)):
        fy3_writefits((im[i][::-1,:]).T,outfitsdir,'',Scale=scale0,obstime=timelist[i],R_SUN=re[i][2],rmghost=1,coe=coe)
